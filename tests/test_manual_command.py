...
import autoscaler.manual_command as manual
import pytest
...
class RunnerScaleTestCase(TestCase):
    ...
    @mock.patch('autoscaler.strategy.manual.ManualRunnersScaler.run')
    def test_main_manual(self, mock_run, mock_namespace, mock_get_uuids, mock_args, mock_validate_kubernetes):
        mock_validate_kubernetes.return_value = mock.Mock(returncode=0)
        mock_validate_kubernetes.return_value.check_returncode = mock.Mock(returncode=0)
        mock_args.return_value = argparse.Namespace(config='tests/strategy/test_config_manual.yaml')
    ...
    @mock.patch('autoscaler.runner.get_bitbucket_workspace_repository_uuids')
    @mock.patch('autoscaler.runner.check_kubernetes_namespace')
    @mock.patch('autoscaler.clients.kubernetes.base.KubernetesBaseAPIService.create_config_map')
    @mock.patch('autoscaler.clients.kubernetes.base.KubernetesSpecFileAPIService.generate_kube_spec_file')
    @mock.patch('autoscaler.clients.kubernetes.base.KubernetesSpecFileAPIService.apply_kubernetes_spec_file')
    @mock.patch('autoscaler.manual_command.open')
    def test_main_automatic(
        ...
        mock_validate_kubernetes.return_value = mock.Mock(returncode=0)
        mock_validate_kubernetes.return_value.check_returncode = mock.Mock(returncode=0)
        mock_args.return_value = argparse.Namespace(config='tests/strategy/test_config_automatic.yaml')
        mock_get_uuids.return_value = ('test-workspace-uuid', 'test-repo-uuid')
